...
export function CompanyTypes({lang}:{lang:Lang}){
  return (
    <Reveal delay={0.2}>
      <LandingAccordionItem lang={lang} />
    </Reveal>
  );
}
        ...
        <div className="grid gap-md md:grid-cols-4">
          {steps.map((s,i)=>(
            <Reveal key={i} delay={0.1 * (i + 1)}>
              <div className="relative flex flex-col items-center text-center group p-md rounded-xl hover:bg-surface-card/50 transition-all h-full">
  ...
  return (
    <section id="pricing" className="mx-auto max-w-content px-md py-section" dir={lang==='ar'?'rtl':'ltr'}>
      ...
      <div className="grid gap-md lg:grid-cols-3">
        {packages.map((p, i)=>(
          <Reveal key={p.name} delay={0.1 * (i + 1)}>
            <div className={`flex flex-col rounded-xl p-lg border transition-all duration-500 hover:shadow-soft-lg h-full ${p.popular?'bg-surface-elevated border-primary/40 shadow-soft-lg scale-[1.02] z-10':'bg-surface-card border-hairline-dark hover:border-muted/20'}`}>
              <div className="mb-md">
      ...
      <div className="mx-auto max-w-content px-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-md">
          <Reveal delay={0.2} className="md:col-span-2">
            <div className="card-official flex flex-col md:flex-row gap-md items-center h-full">
          ...
          <Reveal delay={0.3}>
            <div className="card-official flex flex-col justify-center items-center text-center h-full">
  ...
  return (
    <section className="mx-auto max-w-content px-md py-section" dir={lang==='ar'?'rtl':'ltr'}>
      ...
      <div className="grid gap-md md:grid-cols-3">
        {why.map((w,i)=>(
          <Reveal key={i} delay={0.1 * (i + 1)}>
            <div className="card-official group border-l-2 border-l-primary/20 hover:border-l-primary transition-all h-full">
    ...
    <section id="faq" className="bg-canvas-dark py-section" dir={lang==='ar'?'rtl':'ltr'}>
      <div className="mx-auto max-w-2xl px-md">
        ...
        <div className="space-y-xs">
          {faqs.map((f,i)=>(
            <Reveal key={i} delay={0.1 * (i + 1)}>
              <details className="group rounded-lg border border-hairline-dark bg-surface-card/50 transition-all hover:bg-surface-card">
